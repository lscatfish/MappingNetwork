...
仅导出 `ParameterGenerator` 和 `LinearMappingNetwork`，与 factory 注册的 `GENERATOR_MAP` 保持一致。
...
| P1 阻塞性修复 | 5 | factory 导入失败、base.py NameError、废弃接口名、w_seed 魔数、test_extensibility 删除 |
...
测试结果：`43 passed, 0 failed`（`uv run python3 -m pytest tests/ -v`）
