...
def test_persistent_state_dict_only_trainable(device):
    gen = LinearMappingNetwork(20, 4, alpha=0.01, device=device)
...
def test_load_persistent_state_dict_restores_trainable(device):
    gen = LinearMappingNetwork(20, 4, alpha=0.01, device=device)
...
def test_linear_mapping_network_w_seed_reproducible(device):
    gen1 = LinearMappingNetwork(20, 4, alpha=0.01, device=device, w_seed=123)
    gen2 = LinearMappingNetwork(20, 4, alpha=0.01, device=device, w_seed=123)
    assert torch.allclose(gen1.W_fixed, gen2.W_fixed)
    gen3 = LinearMappingNetwork(20, 4, alpha=0.01, device=device, w_seed=456)
    assert not torch.allclose(gen1.W_fixed, gen3.W_fixed)


def test_multilayer_linear_mapping_network(device):
...
def test_cnn_mapping_network(device):
